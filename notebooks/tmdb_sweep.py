...
filters = dict(config["discover_filter"])

# config.yaml filter modification:
filters["min_vote_count"] = 15

content_type, lang = "movie", "en"
...
probe = fetch_discover_page(
    content_type, lang, 1, tmdb_api_key, probe_session, **filters
)

probe_total_results = probe.get("total_results", 0)
probe_total_pages = probe.get("total_pages", 0)
...
SWEEP_DATE_GTE = filters.pop("date_gte")
SWEEP_DATE_LTE = filters.pop("date_lte")
...
def fetch_only(tmdb_id):
    details = fetch_title_details(content_type, tmdb_id, tmdb_api_key, get_session())
...
windows = resolve_windows_under_cap(probe, SWEEP_DATE_GTE, SWEEP_DATE_LTE)
print(f"\nResolved {len(windows)} date windows")

total_discovered = 0  # titles the sweep SAW (should reconcile ~ probe_total_results)
total_fetched = 0  # titles actually detail-fetched + written (excludes already_known)
...
for gte, lte in windows:
    ...
    for page in range(1, total_pages + 1):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
            futures = {pool.submit(fetch_only, tid): tid for tid in to_fetch}
