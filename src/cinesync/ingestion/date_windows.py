...
def earliest_date_from_probe(probe_payload: dict, content_type: str) -> str | None:
    """
    Read the earliest release/first-air date from a discover probe that
    was sorted ascending by date. Because the sweep always sorts by
    primary_release_date.asc / first_air_date.asc, results[0] of an
    unbounded page-1 probe is the earliest-dated title matching the
    filter -- so its date is the natural floor for windowing, instead of
    hardcoding a floor year.

    Returns 'YYYY-MM-DD' or None if the probe returned no usable date
    (empty results, or a blank/missing date field -- TMDB sometimes has
    titles with an empty release_date, which sort first; the caller
    should fall back to a sane default floor in that case).
    """
    date_field = "release_date" if content_type == "movie" else "first_air_date"
    results = probe_payload.get("results", [])
    for row in results:
        value = row.get(date_field)
        if value:  # non-empty 'YYYY-MM-DD'
            return value
    return None
...
def resolve_windows_under_cap(
    ...
    **params,
    ...
    -> int is a caller-supplied function that returns the total_pages
    TMDB reports for a window's first page (fetch page 1 with that
    window and read payload['total_pages']). content_type,
    probe_session and any extra **params are forwarded through
    unchanged, so this function itself has zero network/API dependency
    and stays unit-testable with a stub probe.
