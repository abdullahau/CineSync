...
- **Data sources & how they're reached:**
  ...
  - **Wikidata SPARQL** — **DONE.** Awards (P166/P1411, title- + person-level), RT slug (P1258), and the Wikipedia article URL, keyed on the Q-id directly (not P345). Plot text is a separate **two-step Wikipedia `api.php`** fetch. See `ingestion/wikidata/`.
...
- **`title_awards`** `(title_id, statement_id, award_name, result, prestige, level, subject, year, source)` — **PK `(title_id, statement_id)`** (Wikidata statement GUID = natural dedup key; full-replace scoped to `source='wikidata'`). `result ∈ {won, nominated}` from P166/P1411; `level ∈ {title, person}` (person-level awards attach via the P1686 "for work" qualifier, with the person name in `subject`); `prestige` is a family tag (`Oscars`/`BAFTA`/…) or NULL. Nominated-excluding-won dedup via `FILTER NOT EXISTS` per subject+award. Counts/prestige rollups derived in pandas, not stored. Low-confidence supplementary signal.
- **`title_wikidata_meta`** `(title_id PK, wikipedia_url, wikidata_fetched_at, wikidata_error)` — 1:1; the single-valued outputs of the Wikidata SPARQL pass plus its resume/error state. `wikidata_fetched_at` is the **done-flag** (a title that won nothing has zero `title_awards` rows, so absence can't mean "not fetched"). `wikipedia_url` feeds the Stage-B plot fetch.
...
- **`__init__.py`** — `USER_AGENT` built from the `EMAIL` env var (contact imported, template in code); shared by the SPARQL + Wikipedia fetchers.
- **`sparql.py`** — WDQS fetch, **keyed on the Q-id directly** (`VALUES ?item { wd:Q… }`). `fetch_awards(session, qids)` and `fetch_links(session, qids)` batch over `rate_limiting.wikidata.batch_size` Q-ids; both `{'bindings'|'error'}` via `paced_request(..., service="wikidata", method="POST")`. Endpoint + UA are module constants.
- **`parse.py`** — `parse_awards(bindings)` → `{qid: [award…]}` (statement_id, result, level, subject, prestige, year), `parse_links(bindings)` → `{qid: {wikipedia_url, rt_slug}}`, `tag_prestige(label)` (label-prefix family match, `None` if untracked).
...
- **Wikidata/Wikipedia:** `titles_missing_wikidata(conn)` → `(title_id, wikidata_id)`; `upsert_wikidata_result(conn, title_id, result)` (one txn across `title_awards` full-replace + `title_wikidata_meta` + opportunistic `title_rt`); `titles_missing_wikipedia_plot(conn)` → `(title_id, wikipedia_url)`; `upsert_wikipedia_plot(conn, title_id, plot, error)` (error preserves existing text).
...
- **`pipelines/wikidata_ingest.py`** — two stages: **A** (SPARQL, batched over Q-ids → `upsert_wikidata_result`) then **B** (Wikipedia plot per resolved URL → `upsert_wikipedia_plot`). Both single-threaded and resumable.
...
- **Wikidata enrichment** (`ingestion/wikidata/`, `pipelines/wikidata_ingest.py`) — awards (`title_awards`, redesigned with `statement_id`/`level`/`subject`/`prestige`), RT slug (`title_rt`, opportunistic from P1258), Wikipedia article URL + `title_wikidata_meta`, and Wikipedia plot (`title_plots.wikipedia_plot`). Keyed on Q-id. Verified against a synthetic DB + live SPARQL/Wikipedia spot-checks.
