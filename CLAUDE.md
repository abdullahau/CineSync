...
- `src/cinesync/` — canonical single-purpose functions. Drivers import `from cinesync.X import Y`.
  ...
  - `ingestion/` — **all ingestion + CRUD lives here, in per-source subpackages:**
    ...
    - `wikidata/` — `sparql.py` (SPARQL fetch, Q-id-keyed), `parse.py`, `wikipedia.py` (plot fetch), `__init__.py` (`USER_AGENT`, contact email from `EMAIL` env)
...
- **Wikidata enrichment keys on the `wikidata_id` Q-id directly** (`VALUES ?item { wd:Q… }`), NOT on IMDb `P345` — avoids the movie/tv namespace collision and one-IMDb-to-many-items. One SPARQL pass writes three tables: `title_awards` (redesigned — PK `(title_id, statement_id)`, with `level ∈ {title,person}`, `subject`, `prestige`; full-replace scoped to `source='wikidata'`), `title_wikidata_meta` (1:1; `wikidata_fetched_at` is the **done-flag** — a title that won nothing has zero award rows, so absence ≠ unfetched), and `title_rt` (RT slug, written **opportunistically** only when Wikidata carries P1258, `source`-tagged for a future search-engine resolver). RT *ratings* still go to `title_scores`, not `title_rt`.
- **Nominated-excluding-won** award dedup uses `FILTER NOT EXISTS` per subject+award (Wikidata logs some wins under both P166 and P1411). **Prestige family** is tagged by award-label prefix in Python (`parse.tag_prestige`), not QID paths — an unmatched label fails visibly as `NULL`. Counts/rollups are derived in pandas, not stored.
- **Wikipedia plot** is a two-step `api.php` fetch (sections list → plot section HTML → `parsel` strip), landing in `title_plots.wikipedia_plot`; `wikipedia_fetched_at`/`wikipedia_error` mirror the IMDb resume/error convention (a plotless article is a terminal success, error preserves existing text).
